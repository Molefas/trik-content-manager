...
export function createAddInspiration(storage) {
...
export function createListInspirations(storage) {
    return tool(async (input) => {
        ...
        if (input.query)
            filterType = 'byQuery';
        else if (input.minScore || input.maxScore)
            filterType = 'byScore';
...
export function createGetInspiration(storage) {
    return tool(async (input) => {
