...
export async function fetchEmailsFromSender(credentials, senderEmail, options = {}) {
...
function extractLinksFromPayload(payload) {
    const links = [];
    if (!payload)
        return links;
    ...
    if (body) {
        ...
        const filtered = matches.filter((url) => !url.includes('unsubscribe') &&
            !url.includes('mailto:') &&
